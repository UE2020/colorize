...
fn colorize_opencv(frame: &mut core::Mat, net: &impl ModuleT, device: Device) -> Result<()> {
    ...
    let new_ab = tch::no_grad(|| -> anyhow::Result<Tensor> {
        let resized_l = l.copy().to_device(device).unsqueeze(0).upsample_bilinear2d(
            [256, 256],
            false,
            None,
            None,
        ...
        let out = out
            .upsample_bilinear2d([w as i64, h as i64], false, None, None)
            .squeeze();
        Ok(out)
    })?;
...
fn main() -> Result<()> {
    ...
    match args[1].as_str() {
        "train" => {
            ...
            for epoch in 1..=30 {
                ...
                for images in images.chunks(16) {
                    ...
                    let fake_preds = discriminator_net.forward(&fake_image, true);
        ...
        "test" => {
            ...
            let (full_l, full_ab) = load_lab(&args[3], false)?;
            ...
            tch::no_grad(|| -> anyhow::Result<()> {
                ...
                lab_to_rgb(&full_l.squeeze(), &full_ab)?.save("old.png")?;
                Ok(())
            })?;
        }
        "video" => {
